...
**Skill 加载**:
- 工作流任务先加载 `/.claude/skills/aibridge-development-workflow/SKILL.md` 中的 `aibridge-development-workflow`。
- AIBridge Skills 安装在 `/.claude/skills/<skill-name>/SKILL.md`；当本根规则或工作流要求时，从该目录加载同级 Skill。

**项目版本**:
- 当前项目 Unity 版本：2022.3.62f1c1
- 当前项目 C# 语言版本要求：兼容 C# 9.0，禁止使用更高版本语法。

**当前能力状态**:
- Harness 能力快照：`.aibridge/harness/capabilities.json`。RootRule 只提供 compact 摘要；工作流任务需要确认能力时先用 `$CLI harness status` compact 输出，仅在缺失、过期或任务需要未确认能力时读取完整 snapshot 或运行完整探测。已选助手：claude。Skill 根目录：.claude/skills。Code Index：disabled。外部 agent/sub-agent 能力：Unity 无法判断，按 unknown 处理。
